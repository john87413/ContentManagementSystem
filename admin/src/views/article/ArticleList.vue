<template>
    <div>
      ...
      <el-table :data="articles" v-loading="loading">
        ...
        <el-table-column fixed="right" label="操作" width="200">
          <template #default="{ row }">
            <el-button type="primary" @click="editArticle(row._id)">編輯</el-button>
            <el-button type="primary" @click="deleteArticle(row._id, row.name)"
              >刪除</el-button
            >
          </template>
        </el-table-column>
  ...
  const fetchArticles = async (page = 1, limit = 10, nameQuery = "") => {
    try {
      loading.value = true;
      const res = await articleApi.fetchArticles(page, limit, nameQuery);
  ...
  const editArticle = (id) => {
    router.push({ path: `/articles/edit/${id}` });
  };
  ...
  const handlePageChange = (newPage) => {
    ...
    fetchArticles(currentPage.value, pageSize.value, searchForm.value.name);
  };
