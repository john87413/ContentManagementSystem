<template>
  <div>
    ...
    <el-table :data="drinks" v-loading="loading">
      ...
      <el-table-column fixed="right" label="操作" width="200">
        <template #default="{ row }">
          <el-button type="primary" @click="editDrink(row._id)">編輯</el-button>
          <el-button type="primary" @click="deleteDrink(row._id, row.name)"
            >刪除</el-button
          >
        </template>
      </el-table-column>
...
const fetchDrinks = async (page = 1, limit = 10, nameQuery = "") => {
  try {
    loading.value = true;
    const res = await drinkApi.fetchDrinks(page, limit, nameQuery);
...
const editDrink = (id) => {
  router.push({ path: `/drinks/edit/${id}` });
};
...
const deleteDrink = async (id, name) => {
  try {
    ...
    await fetchDrinks(currentPage.value, pageSize.value);
  } catch (error) {
    if (error !== "cancel") {
...
const handlePageChange = (newPage) => {
  ...
  fetchDrinks(currentPage.value, pageSize.value, searchForm.value.name);
};
...
const ClearSearch = () => {
  ...
  currentPage.value = 1;
  fetchDrinks(currentPage.value, pageSize.value);
};
